      ...
      <div class="filter-group">
        ...
        >
          ...
          <option 
            ...
            v-if="batch"
          >
            {{ batch?.name }} <!-- Added optional chaining -->
          </option>
        </select>
      </div>
      <div class="filter-group">
...
export default {
  ...
  setup() {
      ...
      try {
        ...
        try {
          ...
          if (!Array.isArray(recommendations.value)) {
            ...
            toast.error('Error loading recommendations. Unexpected data format received.');
            return
          }
          
          if (recommendations.value.length === 0) {
            console.log('No feed recommendations found for the selected date')
            toast.info('No feed recommendations found for the selected date.');
            return
          }
    ...
    const handleApiError = (error, source = 'API') => {
      ...
      toast.error(`Failed to load ${source.toLowerCase()}. Please try again later.`);
    }
    ...
    const confirmGenerateRecommendations = async () => {
      ...
      try {
        ...
        if (response.data) {
          ...
          toast.success(`Successfully generated feed recommendations.`);
        }
