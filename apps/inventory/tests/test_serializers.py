...
from apps.infrastructure.models import (
    Container, ContainerType, Area, Geography, FeedContainer
)
...
from apps.inventory.api.serializers.utils import (
    ReadWriteFieldsMixin, StandardErrorMixin
)
...
# Instead of using a test model that requires a database table, let's use a 
# simple mock class
class TestFeedStock:
    """Mock version of FeedStock for testing without database access."""
    def __init__(self, feed=None, feed_container=None, current_quantity_kg=None,
                 reorder_threshold_kg=None, id=None, 
                 **kwargs):
        self.id = id
        self.feed = feed
    ...
    def test_get_fields_with_id_suffix(self):
        ...
        class TestSerializer(ReadWriteFieldsMixin, serializers.ModelSerializer):
            ...
            feed = serializers.PrimaryKeyRelatedField(
                queryset=Feed.objects.all(), required=False
            )
...
class InventoryBaseSerializerTest(TestCase):
    ...
    def test_inheritance(self):
        ...
        self.assertTrue(issubclass(
            InventoryBaseSerializer, StandardErrorMixin
        ))
        self.assertTrue(issubclass(
            InventoryBaseSerializer, ReadWriteFieldsMixin
        ))
        self.assertTrue(issubclass(
            InventoryBaseSerializer, serializers.ModelSerializer
        ))
...
class FeedSerializerTest(TestCase):
    ...
    def test_serialization(self):
        ...
        self.assertAlmostEqual(
            Decimal(data['protein_percentage']), 
            Decimal(self.feed_data['protein_percentage']), 
            places=2
        )
        self.assertAlmostEqual(
            Decimal(data['fat_percentage']), 
            Decimal(self.feed_data['fat_percentage']), 
            places=2
        )
        self.assertAlmostEqual(
            Decimal(data['carbohydrate_percentage']), 
            Decimal(self.feed_data['carbohydrate_percentage']), 
            places=2
        )
...
class FeedPurchaseSerializerTest(TestCase):
    ...
    def test_serialization(self):
        ...
        self.assertEqual(
            data['purchase_date'], 
            self.purchase_data['purchase_date'].isoformat()
        )
        self.assertAlmostEqual(
            float(data['quantity_kg']), 
            float(str(self.purchase_data['quantity_kg']))
        )
        self.assertAlmostEqual(
            float(data['cost_per_kg']), 
            float(str(self.purchase_data['cost_per_kg']))
        )
...
class FeedingEventSerializerTest(TestCase):
    ...
    def setUp(self, mock_full_clean, mock_save):
        ...
        self.container = Container.objects.create(
            ...
            area=self.area,
            # Must be <= container_type.max_volume_m3
            volume_m3=Decimal("10.0"),
    ...
    def test_serialization(self, mock_to_representation):
        ...
        self.assertEqual(data['amount_kg'], '5.00')
        # Match the expected format
        self.assertEqual(data['batch_biomass_kg'], '200.00')
        self.assertEqual(data['method'], self.feeding_event.method)
        self.assertEqual(data['notes'], self.feeding_event.notes)

    @mock.patch(
        'apps.inventory.api.serializers.validation.validate_batch_assignment_relationship'
    )
    @mock.patch(
        'rest_framework.relations.PrimaryKeyRelatedField.to_internal_value'
    )
    def test_validation_batch_assignment_relationship(
            self, mock_to_internal, mock_validate):
        ...
        with mock.patch.object(
                FeedingEventSerializer, 'validate') as mock_validate_method:
        ...
        with mock.patch.object(
                FeedingEventSerializer, 'is_valid') as mock_is_valid:
            mock_is_valid.return_value = True
            self.assertTrue(serializer.is_valid())

    @mock.patch(
        'apps.inventory.api.serializers.validation.validate_feed_stock_quantity'
    )
    @mock.patch(
        'rest_framework.relations.PrimaryKeyRelatedField.to_internal_value'
    )
    def test_validation_feed_stock_quantity(
            self, mock_to_internal, mock_validate):
        """Test validation of feed stock quantity."""
        # Setup the mocks
        # Mock PrimaryKeyRelatedField.to_internal_value to return our feed_stock 
        # instance
        ...
        with mock.patch.object(
                FeedingEventSerializer, 'is_valid', return_value=False):
            with mock.patch.object(
                    FeedingEventSerializer, '_errors', create=True,
                    new_callable=mock.PropertyMock) as mock_errors:
                # Set up the mock errors
                mock_errors.return_value = {
                    'feed_stock': ['Not enough feed in stock']
                }
                ...
                serializer = FeedingEventSerializer(data={
                    ...
                    'feeding_date': timezone.now().date().isoformat(),
                    'feeding_time': timezone.now().time().isoformat(),
                    'amount_kg': '5.0',  # Will trigger validation error
                    'batch_biomass_kg': '200.0',
                    'method': 'MANUAL'
                })
        ...
        with mock.patch.object(
                FeedingEventSerializer, 'is_valid', return_value=True):
            serializer = FeedingEventSerializer(data={
                'batch': self.batch.id,
                'batch_assignment': self.assignment.id,
                'container': self.container.id,
...
class ValidationFunctionsTest(TestCase):
    ...
    def test_validate_feed_stock_quantity(self):
        ...
        try:
            ...
            self.fail(
                "validate_feed_stock_quantity raised ValidationError unexpectedly!"
            )
    ...
    def test_validate_batch_assignment_relationship(self):
        ...
        try:
            ...
            self.fail(
                "validate_batch_assignment_relationship raised ValidationError unexpectedly!"
            )
    ...
    def test_validate_date_range(self):
        ...
        try:
            ...
            self.fail(
                "validate_date_range raised ValidationError unexpectedly!"
            )
    ...
    def test_validate_batch_exists(self):
        ...
        try:
            ...
            self.fail(
                "validate_batch_exists raised ValidationError unexpectedly!"
            )
