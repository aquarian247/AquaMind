...
from apps.inventory.api.serializers.base import FeedingBaseSerializer, TimestampedModelSerializer
from apps.inventory.api.serializers.validation import validate_batch_assignment_relationship, validate_feed_stock_quantity

class FeedingEventSerializer(FeedingBaseSerializer, TimestampedModelSerializer):
    """
    Serializer for the FeedingEvent model.
    
    Provides CRUD operations for feeding events with validation and stock updates.
    """
    ...
    def validate(self, data):
        ...
        if 'batch' in data and 'batch_assignment' in data and data['batch_assignment']:
            validate_batch_assignment_relationship(data['batch'], data['batch_assignment'])
        
        # Validate feed stock quantity
        if 'feed_stock' in data and data['feed_stock'] and 'amount_kg' in data:
