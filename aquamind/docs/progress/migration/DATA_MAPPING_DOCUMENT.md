...
- **Activity Explorer “Input” (GUI‑observed, not yet in extracts):** Appears to encode FW unit → Sea unit moves with **TransportCarrier / trip / compartment** metadata. Relevant FishTalk tables **not currently extracted**: `TransportCarrier`, `TransportMethods`, `Ext_Transporters_v2` (and any Input/Transport operation tables tied to `InternalDelivery.InputOperationID`). `Ext_Inputs_v2.Transporter` exists but is **null** in the 2026‑01‑22 extract. Add these tables to CSV extraction before attempting to use this linkage.
