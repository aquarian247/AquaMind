#!/usr/bin/env python3
# flake8: noqa
"""Pilot migrate FishTalk escape events for one stitched population component.

Source (FishTalk): dbo.Escapes keyed by ActionID.
  - Escapes.ActionID -> Action.PopulationID (+ OperationID -> Operations.StartTime)
  - Escapes.EscapeCauseID -> EscapeCauses.EscapeCausesID (DefaultText)
...
def map_cause(cause_text: str) -> str:
    ...
    if any(token in upper for token in ("DISEASE", "IPN", "ISA", "VIBRIO", "PD ", "CMS", "BKD", "FUNG", "AMOEBA")):
        return "DISEASE"
    if any(token in upper for token in ("HANDLING", "TRANSPORT", "STUNNER", "TRANSFER", "GRADING")):
        return "HANDLING"
...
def build_status_snapshot_index(csv_dir: Path, population_ids: set[str]) -> dict[str, tuple[list[datetime], list[tuple[int, Decimal]]]]:
    ...
    with path.open("r", encoding="utf-8", newline="") as handle:
        ...
        for row in reader:
            ...
            try:
                count = int(round(float(row.get("CurrentCount") or 0)))
            except Exception:
                count = 0
            try:
                biomass = Decimal(str(row.get("CurrentBiomassKg") or 0))
            except Exception:
                biomass = Decimal("0.00")
            raw.setdefault(pop_id, []).append((ts, count, biomass))

    index: dict[str, tuple[list[datetime], list[tuple[int, Decimal]]]] = {}
    for pop_id, rows in raw.items():
        rows.sort(key=lambda item: item[0])
        timestamps = [item[0] for item in rows]
        values = [(item[1], item[2]) for item in rows]
        index[pop_id] = (timestamps, values)
    return index
...
def lookup_status_snapshot_from_index(
    ...
    if population_id not in index:
        return 0, Decimal("0.00")
    timestamps, values = index[population_id]
    if not timestamps:
        return 0, Decimal("0.00")
    # binary search
    from bisect import bisect_right

    pos = bisect_right(timestamps, at_time)
    if pos == 0:
        count, biomass = values[0]
    else:
        count, biomass = values[pos - 1]
    return max(count, 0), biomass


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Pilot migrate escapes for a stitched FishTalk component")
...
def main() -> int:
    ...
    if args.use_csv:
        ...
        rows = loader.get_escape_actions_for_populations(
            set(population_ids),
            start_time=window_start,
            end_time=window_end,
        )
        snapshot_index = build_status_snapshot_index(Path(args.use_csv), set(population_ids))
        extractor = None
    else:
    ...
    if args.dry_run:
        print(f"[dry-run] Would migrate {len(rows)} FishTalk escape rows into batch={batch.batch_number}")
        return 0
    ...
    with transaction.atomic():
        history_reason = f"FishTalk migration: escapes for component {component_key}"
        for row in rows:
            ...
            when = parse_dt(row.get("OperationStartTime") or "")
            if when is None:
                skipped += 1
                continue
            when = ensure_aware(when)

            count_raw = (row.get("EscapeCount") or "").strip()
            try:
            ...
            biomass = to_decimal(row.get("EscapeBiomass"), places="0.01")
            if biomass is None or biomass <= 0:
                if snapshot_index:
                    snap_count, snap_biomass = lookup_status_snapshot_from_index(snapshot_index, population_id, when)
                else:
                    snap_count, snap_biomass = lookup_status_snapshot(
                        population_id=population_id, at_time=when, extractor=extractor
                    )
            ...
            cause_id = (row.get("EscapeCauseID") or "").strip()
            discovery_time = (row.get("DiscoveryTime") or "").strip()
            comment = (row.get("Comment") or "").strip()
            escaped_all = (row.get("EscapedAll") or "").strip()
            cause = map_cause(cause_text)

            description_parts = ["FishTalk escape"]
            if cause_text:
                description_parts.append(f"cause={cause_text}")
            if cause_id and not cause_text:
                description_parts.append(f"EscapeCauseID={cause_id}")
            if discovery_time:
                description_parts.append(f"DiscoveryTime={discovery_time}")
            if escaped_all:
                description_parts.append(f"EscapedAll={escaped_all}")
            if comment:
            ...
            mapped = get_external_map("Escapes", action_id)
            if mapped:
                obj = MortalityEvent.objects.get(pk=mapped.target_object_id)
                for k, v in defaults.items():
                    setattr(obj, k, v)
                save_with_history(obj, user=None, reason=history_reason)
                updated += 1
            else:
        ...
        if args.sync_assignment_counts:
            ...
            for pop_id, assignment in assignment_by_pop.items():
                ...
                save_with_history(
                    assignment,
                    user=None,
                    reason=f"FishTalk escape sync {component_key[:8]}",
                )
                adjusted_assignments += 1

    print(
        f"Migrated escapes for component_key={component_key} into batch={batch.batch_number} "
