#!/usr/bin/env python3
# flake8: noqa
"""Pilot migrate FishTalk culling events for one stitched population component.

Source (FishTalk): dbo.Culling keyed by ActionID.
  - Culling.ActionID -> Action.PopulationID (+ OperationID -> Operations.StartTime)
  - Culling.CullingCauseID -> MortalityCauses.MortalityCausesID (DefaultText)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Pilot migrate culling for a stitched FishTalk component")
...
def main() -> int:
    ...
    if args.use_csv:
        ...
        rows = loader.get_culling_actions_for_populations(
            set(population_ids),
    ...
    if args.dry_run:
        print(f"[dry-run] Would migrate {len(rows)} FishTalk culling rows into batch={batch.batch_number}")
        return 0
    ...
    with transaction.atomic():
        ...
        for row in rows:
            ...
            count_raw = (row.get("CullingCount") or "").strip()
            try:
            ...
            biomass = to_decimal(row.get("CullingBiomass"), places="0.01")
            ...
            cause_id = (row.get("CullingCauseID") or "").strip()
            comment = (row.get("Comment") or "").strip()
            culled_all = (row.get("CulledAll") or "").strip()
            cause = map_cause(cause_text)

            description_parts = ["FishTalk culling"]
            if cause_text:
                description_parts.append(f"cause={cause_text}")
            if cause_id and not cause_text:
                description_parts.append(f"CullingCauseID={cause_id}")
            if culled_all:
                description_parts.append(f"CulledAll={culled_all}")
            if comment:
            ...
            mapped = get_external_map("Culling", action_id)
            if mapped:
